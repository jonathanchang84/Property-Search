...
The application utilizes a decoupling pattern separating the client execution environment, computational service layers, and the persistent storage backend.
+-----------------------------------------------------------------------------------+
|                                STREAMLIT FRONTEND                                 |
|  +--------------------------------------------+  +-----------------------------+  |
...
|   +--------------------+     +---------------------+     +--------------------+   |
|   |   BeautifulSoup    |     |     Gemini API      |     |  GeoPy Geolocator  |   |
|   |  (DOM Harvesting)  |     | (Structured Schema) |     | (Spatial Lookups)  |   |
|   +--------------------+     +---------------------+     +--------------------+   |
+-----------------------------------------+-----------------------------------------+
                                          |
...
Workspace B: Portfolio Map Explorer
	...
	2.	Multi-Parameter Filtering Matrix: The workspace feeds the pulled dataset through a four-stage console to refine records live across multiple dimensions:
